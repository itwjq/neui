...
$("a[name='add_partner']").click(function(){
    ...
    if(inArray(uid_choose,u_choose)){
        ...
        new_tr.find("td[class='td-fir']").find("font").attr('id',uid_choose);//显示的用户名
        new_tr.find("td[class='td-fir']").find("input").val(uid_choose);//显示的用户名
        new_tr.find("td[class='td-third']").find("textarea").val(u_do);//显示的用户名
        $("#add_userdo").val('');
        $("#pro_partners").prepend(new_tr);
    }
});
//实时搜索

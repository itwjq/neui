...
// die;
	...
	$the_authers =  query_posts("auther={$_POST['post_author']}{$_POST['post_author']}");
	// var_dump($_POST['post_author']);
	// var_dump($the_authers);
	foreach($the_authers as $k=>$v){
		if($_POST['post_title'] == $v->post_title && $_POST['excerpt'] == $v->post_excerpt){
	...
	if($pid){
		...
		if(count($_POST['pro_user'])>0){
			foreach($_POST['pro_user'] as $k=>$v){
				...
				echo "add_post_meta({$pid}, 'pro_users', {$v}#-user--do-#{$_POST['pro_userdo'][$k]})";
				// $post_meta_arr['pro_users'] = $v.'#-user--do-#'.$_POST['pro_userdo'][$k];
			}
		}
